        ...
        <script>
            ...
            function calculateData() {
                ...
                for (let i = 0; i < main_interation.length; i++) {
                    ...
                    if ((Number($('#marks_' + main_interation[i].id).val()) < Number($('#pass_' + main_interation[i].id)
                            .val()) || ($('#pass_' + main_interation[i].id).val() == 'F'))) {
                        failed_subjects.push({
                            id: main_interation[i].id,
                            name: main_interation[i].name
                        });
                    }
                }
                        ...
                        success: function(data) {
                            if (data.length < 1) {
                                ...
                                for (var i = 0; i < data.subjects.length; i++) {
                                    subjects.append(
                                        '<div class="form-inline mt-2"><label class="col-sm-2" for="marks_' +
                                        data.subjects[i].id + '">' + data.subjects[i].name +
                                        ' Marks</label><input id="marks_' + data.subjects[i].id +
                                        '" type="number" placeholder="Enter ' + data.subjects[i].name +
                                        ' Marks " class="form-control"><input type="number" id="pass_' +
                                        data.subjects[i].id + '" placeholder="Enter ' + data.subjects[i]
                                        .name +
                                        ' Pass Marks " class="form-control ml-4"><input type="number" id="full_' +
                                        data.subjects[i].id + '" placeholder="Enter ' + data.subjects[i]
                                        .name +
                                        ' Full Marks " class="form-control ml-4"><select class="form-control ml-4" id="grade_' +
                                        data.subjects[i].id +
                                        '"><option value="A+">A+</option><option value="A">A</option><option value="B+">B+</option><option value="B">B</option><option value="C+">C+</option><option value="C">C</option><option value="D">D</option><option value="F">F</option></select></div>'
                                    );
                                }
                                subjects.append(
                                    '<button onclick="calculateData();" type="button" id="calculate" class="float-center ml-4 mt-2 btn btn-primary">Calculate</button>'
                                );
                                subjects.change();
                            }
                        }
                        ...
                        success: function(data) {
                            if (data.exams.length < 1) {
                                ...
                                for (var i = 0; i < data.exams.length; i++) {
                                    exams.append('<option value=' + data.exams[i].id + '>' + data.exams[i]
                                        .value + '</option>');
                                }
                                students.change();
                                exams.change();
